...
2. **Process phase** (per-message, short transactions): For each claimed message, run the handler (dispatch to broker) and update the message state (`processed_at`, clear `claimed_at`) in its own short transaction. If processing fails, the message's `claimed_at` remains set.
