...
func UntilSuccessOr5Failures(failMessage string, fn interface{}, params ...interface{}) []reflect.Value {
	...
	if f.Type().NumIn() != len(params) {
		panic(fmt.Sprintf("RetryUntilSuccessOr5: %s: incorrect number of parameters provided vs required!", failMessage))
	}
	...
	for nRetry := 1; nRetry <= retryLimit; nRetry++ {
		...
		if errorValue.IsNil() {
			if nRetry > 1 {
				fmt.Printf("RetryUntilSuccessOr5: %s succeeded after %d retries!\n", failMessage, nRetry)
			}
			return result
		}

		if nRetry < retryLimit {
			fmt.Printf("RetryUntilSuccessOr5: %s failed (%d/%d), retrying in 1 second...\n", failMessage, nRetry, retryLimit)
			time.Sleep(retryDelay)
		} else {
			err := errorValue.Interface().(error)
			fmt.Printf("RetryUntilSuccessOr5: %s failed (%d/%d), giving up: %s\n", failMessage, nRetry, retryLimit, err.Error())
		}
	}
