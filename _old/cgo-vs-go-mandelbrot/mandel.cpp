...
{
public:
    ParallelMandelbrotGenerator(size_t width, size_t height, RGBA *destImg, int xmin = -2, int ymin = -2, int xmax = 2, int ymax = 2) : width(width), height(height), img(destImg), xmin(xmin), ymin(ymin), xmax(xmax), ymax(ymax) {}
...
private:
    void processParallelInSegments(int numSegments)
    {
        ...
        for (int nSegment = 0; nSegment < numSegments; nSegment++)
        {
            ...
            const auto procesSegment_ = [this, yBegin, yEnd]()
            { processSegment(yBegin, yEnd); };
